...
createApp({
  setup() {
    ...
    const isSuperAdmin = ref(false); // 專屬管理員 (180156)
    ...
    const ghConfig = reactive({
      owner: '',
      repo: '',
      branch: 'main',
      token: '',
      editorPassword: GitHubSync.DEFAULT_EDITOR_PASSWORD,
      superAdminPassword: GitHubSync.DEFAULT_SUPER_ADMIN_PASSWORD,
    });
    ...
    function getItemDeadlineInfo(item, targetDate = currentDate.value) {
      ...
      if (diff === 0) {
        ...
        return {
          ...
          badgeClass: diff === 1 ? 'bg-amber-100 text-amber-900 border-amber-300 font-bold' : 'bg-amber-50 text-amber-800 border-amber-200',
    ...
    // 依據當前選定日期的項目清單（支援多日跨期同步，直到期限結束）
    const currentRecords = computed(() => {
      if (viewMode.value === 'daily') {
        return records.value.filter((r) => isItemActiveOnDate(r, currentDate.value));
      } else {
        // 近期所有未過期項目（以當前日期為基準）
        return records.value.filter((r) => !r.dueDate || r.dueDate >= currentDate.value);
      }
    });
    ...
    async function loadData() {
      ...
      try {
        ...
        if (res.success && res.data) {
          ...
          showToast('無法載入聯絡簿資料，請確認 records.json 是否存在', 'error');
        }
      } catch (err) {
    ...
    async function saveAllData() {
      ...
      // 自動補齊 owner 與 repo（若尚未手動設定）
      if (!ghConfig.owner || !ghConfig.repo) {
        const auto = GitHubSync.autoDetectRepoInfo();
        if (auto.owner) ghConfig.owner = auto.owner;
        if (auto.repo) ghConfig.repo = auto.repo;
      }

      // 如果尚未配置 Token
      if (!ghConfig.token) {
        showToast('初次使用請先填入一次 GitHub Token，之後便能直接一鍵同步！', 'info');
        showConfigModal.value = true;
        return;
      }

      if (!ghConfig.owner || !ghConfig.repo) {
        showToast('請先填入 GitHub 帳號與倉庫名稱', 'info');
        showConfigModal.value = true;
        return;
      }
      ...
      try {
        const res = await GitHubSync.saveData(payload, ghConfig);
        if (res.success) {
          showToast('🎉 成功直接同步發布至 GitHub 倉庫！', 'success');
        }
      } catch (err) {
    ...
    function openAddModal(defaultCategory = 'homework') {
      ...
      formItem.subject = defaultCategory === 'homework' ? '國語' : defaultCategory === 'exam' ? '數學' : '';
      formItem.title = '';
      formItem.details = '';
    ...
    function openSuperAdminModal() {
      passwordModalTitle.value = '專屬管理員驗證';
      passwordModalSubtitle.value = '請輸入專屬管理員密碼以解鎖公告與密碼設定';
      inputPassword.value = '';
      loginError.value = '';
      showPasswordModal.value = true;
    }
    ...
    function submitPassword() {
      const role = GitHubSync.verifyPassword(inputPassword.value);
      if (role === 'super_admin') {
        GitHubSync.setSessionAuth('super_admin');
        isAdmin.value = true;
        isSuperAdmin.value = true;
        showPasswordModal.value = false;
        inputPassword.value = '';
        loginError.value = '';
        showToast('👑 專屬管理員驗證成功！已解鎖公告與設定功能', 'success');
        updateAdminLinks();
      } else if (role === 'editor') {
        GitHubSync.setSessionAuth('editor');
        isAdmin.value = true;
    ...
    function logoutAdmin() {
      GitHubSync.setSessionAuth(null);
      isAdmin.value = false;
      isSuperAdmin.value = false;
    ...
    const directSyncEditorLink = ref('');

    function updateAdminLinks() {
      const base = window.location.origin + window.location.pathname;
      const editorPwd = ghConfig.editorPassword || GitHubSync.DEFAULT_EDITOR_PASSWORD;
      const superPwd = ghConfig.superAdminPassword || GitHubSync.DEFAULT_SUPER_ADMIN_PASSWORD;
      simplePasswordLink.value = `${base}#${editorPwd}`;
      superAdminLink.value = `${base}#${superPwd}`;
      generatedAdminLink.value = GitHubSync.generateAdminLink(ghConfig);
      directSyncEditorLink.value = GitHubSync.generateDirectSyncEditorLink(ghConfig);
    }

    // 儲存 GitHub 設定與產生專屬管理員連結
    function saveGitHubSettings() {
      GitHubSync.saveConfig(ghConfig);
      isAdmin.value = true;
      updateAdminLinks();
    ...
    function copyAdminLink() {
      if (!generatedAdminLink.value) {
        updateAdminLinks();
      }
      navigator.clipboard.writeText(generatedAdminLink.value).then(() => {
        showToast('🔗 已複製管理員完整專屬連結！', 'success');
      });
    }
    ...
    function copySuperAdminLink() {
      ...
      navigator.clipboard.writeText(superAdminLink.value).then(() => {
        showToast('👑 已複製專屬管理員免密碼連結！', 'success');
      });
    }

    function copyDirectSyncEditorLink() {
      if (!directSyncEditorLink.value) {
        updateAdminLinks();
      }
      navigator.clipboard.writeText(directSyncEditorLink.value).then(() => {
        showToast('🚀 已複製「直接同步免設定」編輯連結！協作者點開即可直接發布', 'success');
      });
    }
    ...
    function downloadBackup() {
      ...
      GitHubSync.downloadBackup(payload);
    ...
    onMounted(() => {
      // 解析身分與設定
      const effective = GitHubSync.getEffectiveConfig();
      ghConfig.owner = effective.owner;
      ghConfig.repo = effective.repo;
      ghConfig.branch = effective.branch || 'main';
      ghConfig.token = effective.token;
      ghConfig.editorPassword = effective.editorPassword;
      ghConfig.superAdminPassword = effective.superAdminPassword;
    ...
    return {
      ...
      superAdminLink,
      copySuperAdminLink,
      directSyncEditorLink,
      copyDirectSyncEditorLink,
      getItemDeadlineInfo,
      toast,
      currentRecords,
      homeworkItems,
