...
createApp({
  setup() {
    ...
    const logsTableNotFound = ref(false);
    ...
    async function fetchHistoryLogs() {
      ...
      try {
        const res = await SupabaseSync.fetchLogs(sbConfig);
        if (res.success) {
          logsList.value = res.data;
        } else if (res.tableNotFound) {
          logsTableNotFound.value = true;
          logsList.value = [];
        } else {
          showToast('無法取得歷史紀錄：' + (res.message || '未知錯誤'), 'error');
        }
      } catch (err) {
        showToast('查詢歷史紀錄失敗：' + err.message, 'error');
      } finally {
    ...
    function toggleLogPreview(logItem) {
      if (selectedLogForPreview.value && selectedLogForPreview.value.id === logItem.id) {
        ...
        selectedLogForPreview.value = logItem;
      }
    }
    ...
    async function restoreSnapshot(logItem) {
      ...
      const ok = confirm(`⚠️ 確定要將聯絡簿資料還原至【${timeInfo.exact}】（${timeInfo.relative}）的版本嗎？\n\n注意：當前所有的資料將會被該歷史版本取代，並立即自動發布至雲端推播！`);
      if (!ok) return;

      isRestoring.value = true;
      try {
        const restoredTitle = logItem.class_title || '班級聯絡簿';
        ...
        const res = await SupabaseSync.saveData(sbConfig, payload, restoreNote);
        if (res.success) {
          showToast(`🎉 成功還原至 ${timeInfo.exact} 的版本！`, 'success');
          showLogsModal.value = false;
        }
      } catch (err) {
        showToast('還原版本失敗：' + err.message, 'error');
      } finally {
